...
class ImageRepositoryCustomImplTest extends IntegrationTestSupport {
    ...
    @DisplayName("조건이 animal 일 때, 멤버 아이디를 통해 이미지를 조회한다.")
    @Test
    void findAnimalImageByMemberId() throws Exception {
        // given
        Member member = Member.builder()
                .uid("12345-asdf")
                .email("gunhong951@gmail.com")
        ...
        filterCondition.setKingdom("animal");

        // when
        PageImpl<ImageDto> imageByMemberId = imageRepository.findImageByMemberId(member.getId(), filterCondition, pageable);
        List<ImageDto> contents = imageByMemberId.getContent();

        // then
        assertThat(imageByMemberId.getTotalElements()).isEqualTo(10);
        assertThat(imageByMemberId.getSize()).isEqualTo(10);
        for (ImageDto content : contents) {
            assertThat(content.getImageId()).isNotNull();
            assertThat(content.getFullPath()).isNotNull();
